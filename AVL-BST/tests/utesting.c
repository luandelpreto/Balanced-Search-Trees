...
static void _AVL_insert_increasing_order(AVL_bst *tree);
static void _AVL_insert_unordered(AVL_bst *tree, size_t n, const int ints[n]);
...
static void TEST_INT_AVL_insert_increasing_order(void **state)
{
    AVL_bst *tree = AVL_create();
    assert(tree);
    assert_int_equal(AVL_init(tree, compare_int, alloc, NULL, free), AVL_SUCCESS);

    int *intptrs[4];
    for (size_t i = 0; i < 4; i++) {
        intptrs[i] = malloc(sizeof *intptrs[i]);
        *intptrs[i] = i+1;
        assert_int_equal(AVL_insert(tree, intptrs[i], intptrs[i]), AVL_SUCCESS);
    }
    ...
    *ptr = 5;
    assert_int_equal(AVL_insert(tree, ptr, ptr), AVL_SUCCESS);
    assert_int_equal(tree->root->height, 3);
    *ptr = 3;
#ifdef AVL_ONLY_DISTINCT_KEYS
    assert_int_equal(AVL_insert(tree, ptr, ptr), AVL_DISTINCT_KEY_ERR);
#else
    int *newdata = alloc(sizeof *newdata);
    *newdata = 87;
    assert_int_equal(AVL_insert(tree, ptr, newdata), AVL_SUCCESS);
    tmp = (void *) tree->root->subtrees[1]->subtrees[0]->subtrees[0];
    assert_int_equal(*((int *) tmp), 87);
#endif

    assert_int_equal(AVL_clear(tree), AVL_SUCCESS);
    assert_int_equal(AVL_destroy(&tree), AVL_SUCCESS);
}
...
static void _AVL_insert_increasing_order(AVL_bst *tree)
{
    ...
    for (size_t i = 1; i <= NTESTS; i++) {
        ...
        *ptrs[i] = i;
        assert(AVL_insert(tree, ptrs[i], ptrs[i]) == AVL_SUCCESS);
    }
}
...
static void TEST_INT_AVL_find(void **state)
{
    ...
    AVL_bst_node *f = AVL_find(tree, &(int){1});
    assert(!f);
    ...
    for (size_t i = 1; i <= NTESTS; i++) {
        ...
        *ptr = i;
        AVL_bst_node *f = AVL_find(tree, ptr);
        assert_int_equal(*((int *) f->subtrees[0]), i);
        free(ptr);
    }

    f = AVL_find(tree, &(int){-3});
    assert(!f);
...
static void TEST_INT_AVL_interval_find(void **state)
{
    ...
    *a = -32; *b = 756;
    ...
    AVL_bst_node *lst = AVL_interval_find(tree, a, b);
    assert(lst);
    free(a);
    free(b);
    ...
    size_t i = 0;
    for (AVL_bst_node *p = lst; p; p = p->subtrees[1]) {
        assert_int_equal(*((int *) p->subtrees[0]), interval[i++]);
    }

    AVL_clear_node_list(tree, lst);
    assert_int_equal(AVL_clear(tree), AVL_SUCCESS);
    assert_int_equal(AVL_destroy(&tree), AVL_SUCCESS);
}
...
static void TEST_INT_AVL_delete(void **state)
{
    ...
    assert_int_equal(AVL_delete(tree, &(int){137}, &deldata[0]), AVL_SUCCESS);
    assert_int_equal(*((int *) deldata[0]), 137);
    assert_int_equal(tree->root->subtrees[1]->subtrees[0]->height, 1);

    assert_int_equal(AVL_delete(tree, &(int){-32}, &deldata[1]), AVL_SUCCESS);
    assert_int_equal(*((int *) deldata[1]), -32);
    assert_int_equal(tree->root->subtrees[0]->height, 2);

    assert_int_equal(AVL_delete(tree, &(int){5456}, &deldata[2]), AVL_SUCCESS);
    assert_int_equal(*((int *) deldata[2]), 5456);
    assert_int_equal(tree->root->subtrees[1]->subtrees[1]->subtrees[1]->height, 0);

    assert_int_equal(AVL_delete(tree, &(int){2098}, &deldata[3]), AVL_SUCCESS);
    assert_int_equal(*((int *) deldata[3]), 2098);
    assert_int_equal(tree->root->subtrees[1]->subtrees[1]->height, 1);

    void *nda = NULL;
    assert_int_equal(AVL_delete(tree, &(int){137}, &nda), AVL_DELETE_NON_EXISTENT_KEY_ERR);
    assert_int_equal(AVL_delete(tree, &(int){-32}, &nda), AVL_DELETE_NON_EXISTENT_KEY_ERR);
    assert_int_equal(AVL_delete(tree, &(int){5456}, &nda), AVL_DELETE_NON_EXISTENT_KEY_ERR);
    assert_int_equal(AVL_delete(tree, &(int){2098}, &nda), AVL_DELETE_NON_EXISTENT_KEY_ERR);
...
static void TEST_USER_DEF_TYPE_insert(void **state)
{
    AVL_bst *tree = AVL_create();
    assert(tree);
    AVL_init(tree, compare_str, alloc, NULL, free);
    ...
    for (size_t i = 0; i < NTESTS; i++) {
        assert_int_equal(AVL_insert(tree, &arr[i]->name, arr[i]), AVL_SUCCESS);
    }
...
static void TEST_USER_DEF_TYPE_interval_find(void **state)
{
    ...
    AVL_bst_node *lst = AVL_interval_find(tree, names[1], names[5]);
    assert_non_null(lst);
    AVL_bst_node *p = lst;
    ...
    lst = AVL_interval_find(tree, names[5], names[1]);
    assert_null(lst);
...
static void TEST_USER_DEF_TYPE_delete(void **state)
{
    ...
    void *ptr[2];
    assert_int_equal(AVL_delete(tree, names[3], &ptr[0]), AVL_SUCCESS);
    ...
    ptr[1] = NULL;
    assert_int_equal(AVL_delete(tree, names[3], &ptr[1]), AVL_DELETE_NON_EXISTENT_KEY_ERR);
    assert_null(ptr[1]);

    assert_int_equal(AVL_delete(tree, names[4], &ptr[1]), AVL_SUCCESS);
