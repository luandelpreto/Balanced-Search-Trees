...
static void TEST_INT_AVL_insert_increasing_order(void **state)
{
    ...
    assert(tree);
    assert_int_equal(Lor_AVL_init(tree, compare_int, alloc, NULL, free), LOR_AVL_SUCCESS);

    int *intptrs[4];
    for (size_t i = 0; i < 4; i++) {
        intptrs[i] = malloc(sizeof *intptrs[i]);
        *intptrs[i] = i+1;
        assert_int_equal(Lor_AVL_insert(tree, intptrs[i], intptrs[i]), LOR_AVL_SUCCESS);
    }
    ...
    *ptr = 5;
    assert_int_equal(Lor_AVL_insert(tree, ptr, ptr), LOR_AVL_SUCCESS);
    assert_int_equal(tree->root->height, 3);
    *ptr = 3;
#ifdef LOR_AVL_ONLY_DISTINCT_KEYS
    assert_int_equal(Lor_AVL_insert(tree, ptr, ptr), LOR_AVL_DISTINCT_KEY_ERR);
#else
    int *newdata = alloc(sizeof *newdata);
    *newdata = 87;
    assert_int_equal(Lor_AVL_insert(tree, ptr, newdata), LOR_AVL_SUCCESS);
    tmp = (void *) tree->root->subtrees[1]->subtrees[0]->subtrees[0];
    assert_int_equal(*((int *) tmp), 87);
#endif

    assert_int_equal(Lor_AVL_clear(tree), LOR_AVL_SUCCESS);
    assert_int_equal(Lor_AVL_destroy(&tree), LOR_AVL_SUCCESS);
}
...
static void _AVL_insert_increasing_order(Lor_AVL_bst *tree)
{
    ...
    for (size_t i = 1; i <= NTESTS; i++) {
        ...
        *ptrs[i] = i;
        assert(Lor_AVL_insert(tree, ptrs[i], ptrs[i]) == LOR_AVL_SUCCESS);
    }
}
...
static void TEST_INT_AVL_delete(void **state)
{
    ...
    assert_int_equal(Lor_AVL_delete(tree, &(int){137}, &deldata[0]), LOR_AVL_SUCCESS);
    assert_int_equal(*((int *) deldata[0]), 137);
    assert_int_equal(tree->root->subtrees[1]->subtrees[0]->height, 1);

    assert_int_equal(Lor_AVL_delete(tree, &(int){-32}, &deldata[1]), LOR_AVL_SUCCESS);
    assert_int_equal(*((int *) deldata[1]), -32);
    assert_int_equal(tree->root->subtrees[0]->height, 2);

    assert_int_equal(Lor_AVL_delete(tree, &(int){5456}, &deldata[2]), LOR_AVL_SUCCESS);
    assert_int_equal(*((int *) deldata[2]), 5456);
    assert_int_equal(tree->root->subtrees[1]->subtrees[1]->subtrees[1]->height, 0);

    assert_int_equal(Lor_AVL_delete(tree, &(int){2098}, &deldata[3]), LOR_AVL_SUCCESS);
    assert_int_equal(*((int *) deldata[3]), 2098);
    assert_int_equal(tree->root->subtrees[1]->subtrees[1]->height, 1);

    void *nda = NULL;
    assert_int_equal(Lor_AVL_delete(tree, &(int){137}, &nda), LOR_AVL_DELETE_NON_EXISTENT_KEY_ERR);
    assert_int_equal(Lor_AVL_delete(tree, &(int){-32}, &nda), LOR_AVL_DELETE_NON_EXISTENT_KEY_ERR);
    assert_int_equal(Lor_AVL_delete(tree, &(int){5456}, &nda), LOR_AVL_DELETE_NON_EXISTENT_KEY_ERR);
    assert_int_equal(Lor_AVL_delete(tree, &(int){2098}, &nda), LOR_AVL_DELETE_NON_EXISTENT_KEY_ERR);
...
static void TEST_USER_DEF_TYPE_insert(void **state)
{
    ...
    for (size_t i = 0; i < NTESTS; i++) {
        assert_int_equal(Lor_AVL_insert(tree, &arr[i]->name, arr[i]), LOR_AVL_SUCCESS);
    }
...
static void TEST_USER_DEF_TYPE_delete(void **state)
{
    ...
    void *ptr[2];
    assert_int_equal(Lor_AVL_delete(tree, names[3], &ptr[0]), LOR_AVL_SUCCESS);
    ...
    ptr[1] = NULL;
    assert_int_equal(Lor_AVL_delete(tree, names[3], &ptr[1]), LOR_AVL_DELETE_NON_EXISTENT_KEY_ERR);
    assert_null(ptr[1]);

    assert_int_equal(Lor_AVL_delete(tree, names[4], &ptr[1]), LOR_AVL_SUCCESS);
